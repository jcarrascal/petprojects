 ...
 * Message Controller
 */
class MassMailerControllerImport extends JController
{
	function upload()
	{
		$rows = array();
		...
		foreach (file($_FILES['csv_file']['tmp_name']) as $line) {
			...
			if (count($fields) !== 5) {
				$this->setMessage(JText::sprintf('COM_MASSMAILER_IMPORT_INVALID_CSV', $lineNumber));
				$this->setRedirect('index.php?option=com_massmailer&view=import&tmpl=component');
				$isValid = false;
				break;
			}
			$rows[] = array(
			...
			if (!preg_match('/^[0-9]{10}$/', $cellphone)) {
				$this->setMessage(JText::sprintf('COM_MASSMAILER_IMPORT_INVALID_CELLPHONE', $lineNumber));
				$this->setRedirect('index.php?option=com_massmailer&view=import&tmpl=component');
				$isValid = false;
				break;
			}
		...
		if ($isValid) {
			$model = $this->getModel('Import', 'MassMailerModel');
			$this->setRedirect('index.php?option=com_massmailer&view=import&tmpl=component&status=success');
			if ($model->batchImport($rows))
				$this->setMessage(JText::_('COM_MASSMAILER_IMPORT_SUCCESS'));
			else
