...
/**
 * Testimonies Message Model.
 */
class TestimoniesModelMessage extends JModelAdmin
{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Messages', $prefix = 'TestimoniesTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_testimonies.message', 'message', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_testimonies.edit.messages.data', array());
		if (empty($data))
		{
			$data = $this->getItem();
		}
	...
	function getParams()
	{
		return JComponentHelper::getParams('com_testimonies');
	}
	...
	{
		$recipientsModel = new TestimoniesModelRecipients();
		$recipients = array();
		...
		try {
			$client = @new SoapClient('http://panel.testimonies.co/ws/sms.wsdl',
