 ...
 * Testimonies Model
 */
class TestimoniesModelTestimonies extends JModelAdmin
{
	/**
	...
	function validate($form, $data, $group = null)
	{
		...
		if (!preg_match('/^[0-9]{10}$/', $data['cellphone'])) {
			$this->setError(JText::_('COM_TESTIMONIES_VALIDATION_CELLPHONE') . var_export($data['cellphone'], true));
			return false;
		}

		$db = $this->getDBO();
		$query = 'select count(*) counter
			from #__testimonies_recipients
			where cellphone = ' . $db->quote($data['cellphone']);
		$db->setQuery($query);
		$row = $db->loadRow();
		if ($row[0] > 0) {
			$this->setError(JText::_('COM_TESTIMONIES_VALIDATION_REGISTERED'));
			return false;
		}
