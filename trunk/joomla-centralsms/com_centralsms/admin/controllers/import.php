 ...
 * Message Controller
 */
class CentralSMSControllerImport extends JController
{
	function upload()
	{
		$rows = array();
		...
		foreach (file($_FILES['csv_file']['tmp_name']) as $line) {
			...
			if (count($fields) !== 5) {
				$this->setMessage(JText::sprintf('COM_CENTRALSMS_IMPORT_INVALID_CSV', $lineNumber));
			...
			if (!preg_match('/^[0-9]{10}$/', $cellphone)) {
				$this->setMessage(JText::sprintf('COM_CENTRALSMS_IMPORT_INVALID_CELLPHONE', $lineNumber));
				$this->setRedirect('index.php?option=com_centralsms&view=import&tmpl=component');
				$isValid = false;
				break;
			}
